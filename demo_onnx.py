...
def infer_fast2(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    inp_img = np.expand_dims(scaled_img.transpose((2, 0, 1)), axis=0)
    print(inp_img.shape)
    stages_output = net.infer(inp_img)
    # print(stages_output)
...
def run_demo(image_provider, height_size, cpu, track, smooth):
    ...
    for img in image_provider:
        orig_img = img.copy()
        heatmaps, pafs, scale, pad = infer_fast2(onnx_model, img, height_size, stride, upsample_ratio, cpu)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--onnx_model', type=str, default='human-pose-estimation.onnx', help='path to the checkpoint')
    ...
    run_demo(frame_provider, args.height_size, args.cpu, args.track, args.smooth)
